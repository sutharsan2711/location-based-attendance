...
const AdminCarryForwardModal: React.FC<AdminCarryForwardModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-[999] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 overflow-y-auto">
      <div className="relative w-full max-w-4xl max-h-[90vh] flex flex-col bg-white dark:bg-slate-900 rounded-3xl shadow-2xl border border-slate-100 dark:border-slate-800 overflow-hidden animate-in fade-in zoom-in duration-200">
        {/* Header */}
        <div className="px-6 py-4 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-gradient-to-r from-blue-900 via-indigo-950 to-slate-900 text-white shrink-0">
        ...
        <div className="p-6 space-y-5 overflow-y-auto flex-1">
          {/* Notifications */}
          {successMsg && (
            <div className="flex items-center gap-2.5 p-3 rounded-2xl bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800 text-emerald-800 dark:text-emerald-200 text-xs font-semibold">
          ...
          {errorMsg && (
            <div className="flex items-center gap-2.5 p-3 rounded-2xl bg-rose-50 dark:bg-rose-950/40 border border-rose-200 dark:border-rose-800 text-rose-800 dark:text-rose-200 text-xs font-semibold">
          ...
          <div className="bg-slate-50 dark:bg-slate-800/80 p-4 rounded-2xl border border-slate-100 dark:border-slate-700 space-y-3">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="text-xs text-primary-600 dark:text-primary-400 hover:underline font-bold flex items-center gap-1 cursor-pointer"
              >
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
              <div>
                <label className="block text-slate-500 dark:text-slate-400 font-semibold mb-1">From Year</label>
                <select
                  value={fromYear}
                  onChange={(e) => setFromYear(Number(e.target.value))}
                  className="w-full px-3 py-1.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100 font-bold outline-none"
                >
              ...
              <div>
                <label className="block text-slate-500 dark:text-slate-400 font-semibold mb-1">To Year (Target)</label>
                <select
                  value={toYear}
              ...
              <div>
                <label className="block text-slate-500 dark:text-slate-400 font-semibold mb-1">Casual Leave Max Cap</label>
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="w-full px-3 py-1.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100 font-bold outline-none disabled:opacity-50"
                  />
                  <input
                    type="checkbox"
                    ...
                    className="h-4 w-4 rounded text-primary-600 cursor-pointer"
                  />
                </div>
              </div>

              <div>
                <label className="block text-slate-500 dark:text-slate-400 font-semibold mb-1">Sick Leave Max Cap</label>
                <div className="flex items-center gap-2">
                  <input
                    type="number"
                    min={0}
          ...
          {preview && (
            <div className="grid grid-cols-3 gap-3">
              <div className="bg-blue-50/70 dark:bg-blue-950/40 p-3 rounded-2xl border border-blue-100 dark:border-blue-800/60 text-center">
                <span className="text-[10px] font-bold uppercase text-blue-700 dark:text-blue-300">Eligible Employees</span>
                <p className="text-xl font-extrabold text-blue-900 dark:text-blue-100 mt-0.5">{preview.totalEmployees}</p>
              </div>
              <div className="bg-indigo-50/70 dark:bg-indigo-950/40 p-3 rounded-2xl border border-indigo-100 dark:border-indigo-800/60 text-center">
                <span className="text-[10px] font-bold uppercase text-indigo-700 dark:text-indigo-300">Total Days to Carry</span>
                <p className="text-xl font-extrabold text-indigo-900 dark:text-indigo-100 mt-0.5">{preview.totalDaysCarriedForward} Days</p>
              </div>
              <div className="bg-emerald-50/70 dark:bg-emerald-950/40 p-3 rounded-2xl border border-emerald-100 dark:border-emerald-800/60 text-center">
                <span className="text-[10px] font-bold uppercase text-emerald-700 dark:text-emerald-300">Target Year</span>
                <p className="text-xl font-extrabold text-emerald-900 dark:text-emerald-100 mt-0.5">Year {preview.toYear}</p>
              </div>
            </div>
          )}
          ...
          <div className="border border-slate-100 dark:border-slate-800 rounded-2xl overflow-hidden">
            <div className="max-h-64 overflow-y-auto">
              <table className="w-full text-left text-xs">
                <thead className="bg-slate-50 dark:bg-slate-800 text-[10px] font-bold text-slate-400 dark:text-slate-300 uppercase tracking-wider sticky top-0 border-b border-slate-100 dark:border-slate-700">
                  <tr>
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800 text-slate-700 dark:text-slate-300">
                  {preview?.employees.map((emp) => (
                    <tr key={emp.employeeId} className="hover:bg-slate-50/60 dark:hover:bg-slate-800/50">
                      <td className="py-2 px-4 font-semibold text-slate-800 dark:text-slate-200">
                        {emp.employeeName} <span className="text-[10px] text-slate-400 dark:text-slate-500 font-normal">({emp.employeeCode})</span>
                      </td>
                      <td className="py-2 px-4 font-mono">{emp.casualClosing}</td>
                      <td className="py-2 px-4 font-mono text-indigo-600 dark:text-indigo-400 font-bold">+{emp.casualCarried}</td>
                      <td className="py-2 px-4 font-mono">{emp.sickClosing}</td>
                      <td className="py-2 px-4 font-mono text-indigo-600 dark:text-indigo-400 font-bold">+{emp.sickCarried}</td>
                      <td className="py-2 px-4 text-right font-mono font-extrabold text-emerald-700 dark:text-emerald-400">
        ...
        <div className="px-6 py-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between bg-slate-50/50 dark:bg-slate-800/50 shrink-0">
          ...
          <Button
            ...
            className="flex items-center gap-1.5 shadow-md bg-emerald-600 hover:bg-emerald-700 text-white"
          >
