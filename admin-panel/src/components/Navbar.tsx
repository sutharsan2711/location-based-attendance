...
const Navbar: React.FC<NavbarProps> = ({ onToggleMobileMenu }) => {
  ...
  return (
    <header className="sticky top-0 z-40 flex h-16 w-full items-center justify-between border-b border-slate-100 bg-white/95 px-3 sm:px-6 backdrop-blur-md">
      {/* Mobile Menu Button + App Brand */}
      <div className="flex items-center gap-2.5">
        <button
          onClick={onToggleMobileMenu}
          className="md:hidden p-2 rounded-xl text-slate-700 hover:bg-slate-100 transition-colors focus:outline-none cursor-pointer"
          title="Open Menu"
        ...
        <div className="font-extrabold text-slate-800 text-sm md:hidden truncate">
          AttendGPS Admin
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 sm:gap-3 shrink-0">
        {user && (
          <div className="relative" ref={dropdownRef}>
            ...
            <button
              ...
              className="flex items-center gap-2 sm:gap-3 rounded-2xl p-1 sm:p-1.5 pr-2 sm:pr-2.5 transition-all duration-200 hover:bg-slate-50 focus:outline-none border border-transparent hover:border-slate-200 cursor-pointer"
            ...
            {dropdownOpen && (
              <div className="absolute right-0 mt-2 w-64 max-w-[88vw] rounded-2xl bg-white p-3 shadow-xl border border-slate-100 ring-1 ring-slate-900/5 animate-scale-up z-50">
        ...
        <button
          onClick={handleLogout}
          className="flex items-center gap-1.5 rounded-xl border border-rose-100 bg-rose-50/70 px-2.5 sm:px-3 py-1.5 text-xs font-semibold text-rose-600 transition-all duration-200 hover:bg-rose-100 hover:border-rose-200 shadow-sm cursor-pointer"
