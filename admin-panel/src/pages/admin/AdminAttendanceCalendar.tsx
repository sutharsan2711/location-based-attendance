    ...
    <div className="space-y-6 pb-16 animate-fade-in text-slate-800">
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          ...
          <div className="relative">
            ...
            <CalendarIcon className="pointer-events-none absolute right-2.5 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-slate-400" />
          </div>
          ...
          <button
            onClick={handleToday}
            className="px-3.5 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs rounded-xl transition shadow-xs cursor-pointer"
            title="Jump to today"
          >
            Today
          </button>
          ...
          <button
            ...
            disabled={loading}
            className="p-2.5 bg-white hover:bg-slate-50 border border-slate-200 text-slate-600 rounded-xl transition shadow-xs disabled:opacity-50 cursor-pointer"
      ...
      {calendarData && (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3.5">
          {/* Card 1: Active Staff */}
          <div className="bg-white rounded-2xl p-4 border border-slate-100 shadow-xs hover:shadow-md transition">
            ...
            <p className="mt-2 text-2xl font-black text-slate-900">{calendarData.totalEmployees}</p>
            <span className="text-[10px] text-slate-400 font-medium">Eligible for punches</span>
          </div>

          {/* Card 2: Total Logins */}
          <div className="bg-white rounded-2xl p-4 border border-slate-100 shadow-xs hover:shadow-md transition">
            <div className="flex items-center justify-between">
              <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Total Logins</span>
            ...
            <span className="text-[10px] text-emerald-700/80 font-medium">Present punches</span>
          </div>
          ...
          <div className="bg-white rounded-2xl p-4 border border-slate-100 shadow-xs hover:shadow-md transition">
            ...
            <span className="text-[10px] text-rose-700/80 font-medium">Casual/Sick/LOP</span>
          </div>

          {/* Card 4: Work From Home */}
          <div className="bg-white rounded-2xl p-4 border border-slate-100 shadow-xs hover:shadow-md transition">
            <div className="flex items-center justify-between">
              <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">WFH Days</span>
            ...
            <span className="text-[10px] text-blue-700/80 font-medium">Remote days</span>
          </div>

          {/* Card 5: Permissions */}
          <div className="bg-white rounded-2xl p-4 border border-slate-100 shadow-xs hover:shadow-md transition">
            <div className="flex items-center justify-between">
              <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Permissions</span>
            ...
            <span className="text-[10px] text-amber-700/80 font-medium">Short permissions</span>
          </div>
          ...
          <div className="bg-white rounded-2xl p-4 border border-slate-100 shadow-xs hover:shadow-md transition">
            ...
            <span className="text-[10px] text-orange-700/80 font-medium">Grace exceeded</span>
          </div>
        </div>
      )}

      {/* ── Main Calendar Container ── */}
      <div className="bg-white rounded-3xl border border-slate-200/80 shadow-sm p-4 sm:p-6 overflow-hidden">
        ...
        <div className="flex items-center justify-between pb-5 border-b border-slate-100">
          <div className="flex items-center gap-3">
            ...
            <span className="hidden sm:inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-primary-50 text-primary-700 border border-primary-100">
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 py-3 px-1 border-b border-slate-100 text-[11px] font-semibold text-slate-600">
          <div className="flex flex-wrap items-center gap-2">
            <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-emerald-50 text-emerald-700 border border-emerald-100 font-bold">
              <span className="h-2 w-2 rounded-full bg-emerald-500" />
              Logins (Presents)
            </span>
            <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-rose-50 text-rose-700 border border-rose-100 font-bold">
              <span className="h-2 w-2 rounded-full bg-rose-500" />
              Approved Leaves
            </span>
            <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-blue-50 text-blue-700 border border-blue-100 font-bold">
              <span className="h-2 w-2 rounded-full bg-blue-500" />
              Work From Home (WFH)
            </span>
            <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-amber-50 text-amber-700 border border-amber-100 font-bold">
              <span className="h-2 w-2 rounded-full bg-amber-500" />
              Permissions
            </span>
            <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-orange-50 text-orange-700 border border-orange-100 font-bold">
              <span className="h-2 w-2 rounded-full bg-orange-500" />
              Late Check-ins
            </span>
            <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-purple-50 text-purple-700 border border-purple-100 font-bold">
        ...
        {loading && !calendarData ? (
          ...
          <div className="overflow-x-auto pt-4">
            <div className="min-w-[760px]">
              ...
              <div className="grid grid-cols-7 gap-2.5 text-center text-xs font-black uppercase tracking-wider text-slate-400 pb-3">
                {WEEKDAYS.map((w, idx) => (
                  <div
                    key={w}
                    className={`py-1.5 rounded-lg ${
                      idx === 0 || idx === 6 ? 'text-rose-400 font-extrabold' : 'text-slate-500'
                    }`}
                  >
                    {w}
                  </div>
                ...
                {gridCells.map((day, index) => {
                  if (!day) {
                    return (
                      <div
                        key={`empty-${index}`}
                        className="min-h-[120px] rounded-2xl bg-slate-50/40 border border-transparent opacity-30"
                      />
                    );
                  }
                  ...
                  return (
                    <div
                      ...
                      className={`min-h-[125px] p-2.5 rounded-2xl border transition-all duration-150 cursor-pointer flex flex-col justify-between group hover:shadow-md hover:scale-[1.01] ${
                        isToday
                          ? 'bg-primary-50/40 border-primary-400 ring-2 ring-primary-500/20 shadow-xs'
                          : day.isHoliday
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between">
                        <span
                          className={`text-xs font-black h-6 w-6 rounded-full flex items-center justify-center transition ${
                            isToday
                              ...
                              : 'text-slate-800 bg-slate-100 group-hover:bg-primary-100 group-hover:text-primary-800'
                          }`}
                        >
                          {day.day}
                        </span>
                        ...
                        {isToday ? (
                          ...
                          <span className="text-[9px] font-bold truncate max-w-[80px] px-1.5 py-0.5 rounded bg-purple-100 text-purple-800" title={day.holidayName || 'Holiday'}>
                      ...
                      <div className="mt-2 space-y-1">
                        {/* Presents / Logins */}
                        {day.presentCount > 0 && (
                          <div className="text-[10px] font-bold px-1.5 py-0.5 rounded-md bg-emerald-100/80 text-emerald-800 flex items-center justify-between border border-emerald-200/50">
                        ...
                        {/* Leaves */}
                        {day.leaveCount > 0 && (
                          <div className="text-[10px] font-bold px-1.5 py-0.5 rounded-md bg-rose-100/80 text-rose-800 flex items-center justify-between border border-rose-200/50">
                            <span className="flex items-center gap-1">
                        ...
                        {/* Work From Home */}
                        {day.wfhCount > 0 && (
                          <div className="text-[10px] font-bold px-1.5 py-0.5 rounded-md bg-blue-100/80 text-blue-800 flex items-center justify-between border border-blue-200/50">
                            <span className="flex items-center gap-1">
                        ...
                        {/* Permissions */}
                        {day.permissionCount > 0 && (
                          <div className="text-[10px] font-bold px-1.5 py-0.5 rounded-md bg-amber-100/80 text-amber-800 flex items-center justify-between border border-amber-200/50">
                            <span className="flex items-center gap-1">
                        ...
                        {/* Late Check-ins */}
                        {day.lateCount > 0 && (
                          <div className="text-[10px] font-bold px-1.5 py-0.5 rounded-md bg-orange-100/80 text-orange-800 flex items-center justify-between border border-orange-200/50">
                            <span className="flex items-center gap-1">
                        ...
                        {!hasActivity && !day.isWeekend && (
                          <span className="text-[10px] text-slate-300 block text-center py-1.5 font-medium">
                            No Logs
                          </span>
                        )}
                        ...
                        {/* Weekend label */}
                        {!hasActivity && day.isWeekend && (
                          <span className="text-[10px] text-rose-300 block text-center py-1.5 font-bold uppercase tracking-wider">
                            Weekend
                          </span>
                        )}
                      </div>
      ...
      {selectedDay && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-3 sm:p-6 overflow-y-auto">
          <div className="bg-white rounded-3xl shadow-2xl border border-slate-100 w-full max-w-5xl max-h-[92vh] flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="border-b border-slate-100 px-6 bg-white shrink-0 flex flex-col sm:flex-row sm:items-center justify-between gap-3 py-3">
              {/* Tabs */}
              ...
              <div className="relative shrink-0">
                ...
                <input
                  ...
                  className="pl-8 pr-3 py-1.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium text-slate-700 placeholder:text-slate-400 focus:bg-white focus:outline-none focus:ring-2 focus:ring-primary-500/20 w-48 transition"
                />
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-6">
              {modalFilteredEmployees.length === 0 ? (
                <div className="py-12 text-center">
                  ...
                  <p className="text-sm font-bold text-slate-700">No records found for this category</p>
                  ...
                  <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="divide-y divide-slate-50 text-xs font-medium text-slate-600">
                      {modalFilteredEmployees.map((emp: any, idx: number) => {
                        ...
                        return (
                          <tr key={`${emp.id}-${recType}-${idx}`} className="hover:bg-slate-50/60 transition">
                            {/* Employee */}
                            <td className="py-3.5 px-4">
                              <div className="flex items-center gap-3">
                                <div className="h-8 w-8 rounded-xl bg-primary-100 text-primary-700 flex items-center justify-center font-bold text-xs shrink-0">
                                  {emp.name.charAt(0)}
                                </div>
                                <div>
                            ...
                            <td className="py-3.5 px-4">
                              ...
                              {recType === 'LATE' && (
                                <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] font-black bg-orange-50 text-orange-700 border border-orange-200">
                            ...
                            {/* Details / Reason */}
                            <td className="py-3.5 px-4">
                              <p className="text-xs text-slate-700 truncate max-w-xs" title={emp.reason || 'No remarks'}>
