...
const AdminDashboard: React.FC = () => {
  ...
  const kpis = [
    ...
    {
      title: 'Today Logouts',
      value: stats.todayLogout,
      icon: LogOutIconHelper, // wait, let's use check circle or clock or custom icon
      iconName: 'logout',
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        {/* Present vs Absent Area Chart */}
        <Card title="Attendance Analysis (Last 7 Days)" subtitle="Comparison of Present vs Absent employees">
        ...
        {/* Login vs Logout Bar Chart */}
        <Card title="Punch Trends (Last 7 Days)" subtitle="Total check-in punches vs check-out punches">
          <div className="h-80 w-full mt-2">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                ...
                <Tooltip contentStyle={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: '16px' }} />
                <Legend iconType="circle" wrapperStyle={{ fontSize: 12, paddingTop: 10 }} />
                <Bar dataKey="login" name="Check-in Punches" fill="#3b66ff" radius={[4, 4, 0, 0]} barSize={16} />
                <Bar dataKey="logout" name="Check-out Punches" fill="#6366f1" radius={[4, 4, 0, 0]} barSize={16} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </Card>
...
// Helper component for LogOut icon, as we already imported LogOut earlier in Navbar but might need it here.
// Let's import it from lucide-react directly
import { LogOut as LogOutIconHelper } from 'lucide-react';
